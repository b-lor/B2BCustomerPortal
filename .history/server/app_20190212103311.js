    ...
    let ticketCount;
       ...
       if(isNaN(+data.searchText)) {
        ...
        Employee.count({}).then((count) => {
            ticketCount = count;
            Employee.find({$or: [{'resolution': {$regex: '.*' + data.searchText + '.*', $options: 'i'}}, {'status': {$regex: '.*' + data.searchText + '.*', $options: 'i'}}]}).sort(data.sortOptions).skip(data.offset).limit(data.rowCount).exec().then((result) => {
                res.status(200).json({
        ...
        Employee.count({}).then((count) => {
            ticketCount = count;
            Employee.find({ticketId: +data.searchText}).sort(data.sortOptions).skip(data.offset).limit(data.rowCount).exec().then((result) => {
                res.status(200).json({
                    title: 'Fetched Employees',
                    obj: result,
                    count: ticketCount
