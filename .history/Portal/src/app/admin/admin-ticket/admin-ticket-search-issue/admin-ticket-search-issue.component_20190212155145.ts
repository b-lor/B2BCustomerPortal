...
export class AdminTicketSearchIssueComponent implements OnInit {
  ...
  currentPage = 1;
    ...
    constructor(private ticketService: TicketService, private router: Router, private http: Http, private chRef: ChangeDetectorRef) {
      this.getTickets();
     }
  ...
  getTickets() {
    ...
    this.http.post('localhost:3000/filter', JSON.stringify(filterOptions),{headers: headers}).subscribe((data) => {
      this.tickets = data.json().obj;
