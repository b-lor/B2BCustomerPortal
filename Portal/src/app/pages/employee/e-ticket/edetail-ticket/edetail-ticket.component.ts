...
@Component({
  selector: 'app-edetail-ticket',
  templateUrl: './edetail-ticket.component.html',
  styleUrls: ['./edetail-ticket.component.css']
})
export class EdetailTicketComponent implements OnInit {
  tickets = new Ticket();
  trackers: Tracker[] = [];
  ...
  ngOnInit() {
    const ticketSub = this.ticketService
      ...
      .subscribe(tickets => {
        ...
        const trackerSub = this.trackerService
          .getCustomerTracker(this.tickets._id)
          .subscribe(trackers => {
  ...
  onSubmit() {
    ...
    this.trackerService.addTracker(this.trackerDetails).subscribe(
      res => {
